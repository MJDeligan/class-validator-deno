...
export class ValidationExecutor {
    ...
    metadatas.forEach((metadata) => {
      this.metadataStorage
        ...
        .forEach((customConstraintMetadata) => {
          ...
          const validationResult = validatedSubValues.every(Boolean);
          if (!validationResult) {
  ...
  private nestedValidations(
    ...
    metadatas.forEach((metadata) => {
      ...
      if (
        ...
        const targetSchema = typeof metadata.target === "string"
    ...
    return metadatas.forEach((metadata) => {
      if (metadata.context) {
        ...
        if (metadata.type === ValidationTypes.CUSTOM_VALIDATION) {
          const customConstraints = this.metadataStorage
  ...
  private getConstraintType(
    ...
    const type = customValidatorMetadata && customValidatorMetadata.name
