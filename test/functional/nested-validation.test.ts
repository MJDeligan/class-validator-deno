...
Rhum.testSuite("nested validation", function () {
  Rhum.testCase("should not validate missing nested objects", function () {
    class MySubClass {
      @MinLength(5)
      name: string;
    }
    ...
    return validator.validate(model).then((errors) => {
      ...
      expect(errors[1].constraints).toEqual({
        isDefined: "mySubClass should not be null or undefined",
      });
      expect(errors[1].value).toBeUndefined();
    });
  });

  Rhum.testCase("should validate nested objects", function () {
    class MySubClass {
      @MinLength(5)
      name: string;
    }
    ...
    return validator.validate(model).then((errors) => {
      ...
      expect(subError1?.constraints).toEqual({
      ...
      expect(subSubError?.constraints).toEqual({
        minLength: "name must be longer than or equal to 5 characters",
      });
      ...
      expect(subSubSubError3?.target).toEqual(
        model.mySubSubClasses[0][0],
      );
      expect(subSubSubError3?.property).toEqual("name");
      expect(subSubSubError3?.value).toEqual("sub");
      expect(subSubSubError3?.constraints).toEqual({
        minLength: "name must be longer than or equal to 5 characters",
      });
      ...
      expect(subSubSubError4?.target).toEqual(
        model.mySubSubSubClasses[0][0],
      );
      expect(subSubSubError4?.value).toEqual(
        model.mySubSubSubClasses[0][0][0],
      );
      expect(subSubSubError4?.property).toEqual("0");

      //
      const subSubSubSubError4 = subSubSubError4?.children?.[0];
      expect(subSubSubSubError4?.target).toEqual(
        model.mySubSubSubClasses[0][0][0],
      );
  ...
  Rhum.testCase("should validate when nested is not object", () => {
    class MySubClass {
      @MinLength(5)
      name: string;
    }
    ...
    return validator.validate(model).then((errors) => {
      ...
      expect(subError?.constraints).toEqual({
        [ValidationTypes.NESTED_VALIDATION]:
          "nested property mySubClass must be either object or array",
      });
    });
  });

  Rhum.testCase("should validate nested set", () => {
    class MySubClass {
      @MinLength(5)
      name: string;
    }
    ...
    return validator.validate(model).then((errors) => {
      ...
      //
      const subError2 = errors[2]?.children?.[0];
      expect(subError1?.target).toEqual(model.mySubClasses);
      expect(subError1?.property).toEqual("0");
      expect(subError1?.value).toEqual(submodel1);
  ...
  Rhum.testCase("should validate nested map", () => {
    class MySubClass {
      @MinLength(5)
      name: string;
    }
