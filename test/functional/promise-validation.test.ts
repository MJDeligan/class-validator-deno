...
describe("promise validation", function () {
  it("should not validate missing nested objects", function () {
    ...
    return validator.validate(model).then((errors) => {
      ...
      errors[1].constraints.should.be.eql({
  ...
  it("should validate nested objects", function () {
    ...
    return validator.validate(model).then((errors) => {
      return Promise.all([model.mySubClass, model.mySubClasses]).then(
        ([modelMySubClass, modelMySubClasses]) => {
          ...
          subError1.constraints.should.be.eql({
          ...
          subSubError.constraints.should.be.eql({
            minLength: "name must be longer than or equal to 5 characters",
          });
  ...
  it("should validate when nested is not object", () => {
    ...
    return validator.validate(model).then((errors) => {
      ...
      subError.constraints.should.be.eql({
  ...
  it("should validate array promise", function () {
    ...
    return validator.validate(model).then((errors) => {
      return Promise.all([model.arrProperty]).then(
        ([modelArrProperty]) => {
          ...
          errors[0].constraints.should.be.eql({
            minLength:
              "arrProperty must be longer than or equal to 2 characters",
          });
